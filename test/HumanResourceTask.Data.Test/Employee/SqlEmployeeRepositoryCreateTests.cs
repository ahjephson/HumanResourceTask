...
{
    public class SqlEmployeeRepositoryCreateTests
    {
        [Fact]
        public async Task GIVEN_ValidEmployeeRecord_WHEN_CreateEmployeeAsync_THEN_ShouldReturnExpectedEmployee()
        {
            ...
            var actualEmployee = await repository.CreateEmployeeAsync(employeeRecord);

            actualEmployee.Should().BeEquivalentTo(expectedEmployee);
        }

        [Fact]
        public async Task GIVEN_RowsAffectedIsZero_WHEN_CreateEmployeeAsync_THEN_ShouldThrowRepositoryException()
        {
            var mockConnectionFactory = new Mock<IDbConnectionFactory>();
            var mockConnection = new Mock<IDbConnection>();
            var mockDapperWrapper = new Mock<IDapperWrapper>();
            ...
            var exception = await Assert.ThrowsAsync<RepositoryException>(() => repository.CreateEmployeeAsync(employeeRecord));

            exception.Message.Should().Be("Failed to insert the employee record.");
        }

        [Fact]
        public async Task GIVEN_NullRecord_WHEN_CreateEmployeeAsync_THEN_ShouldThrowRepositoryException()
        {
            var mockConnectionFactory = new Mock<IDbConnectionFactory>();
            var mockConnection = new Mock<IDbConnection>();
            var mockDapperWrapper = new Mock<IDapperWrapper>();
            ...
            exception.Message.Should().Be("The inserted employee record could not be retrieved.");
        }

        [Fact]
        public async Task GIVEN_SqlException_WHEN_CreateEmployeeAsync_THEN_ShouldThrowRepositoryException()
        {
            var mockConnectionFactory = new Mock<IDbConnectionFactory>();
            var mockConnection = new Mock<IDbConnection>();
            var mockDapperWrapper = new Mock<IDapperWrapper>();
